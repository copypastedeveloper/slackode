...
const FAMILIES: Family[] = [
  ...
  },
  {
    ...
    note: "Defaults to the current channel. Use `--all` for the whole workspace, or `--channel #other` for a specific one.",
    lines: [
      "• `stats` — current channel, last 24h",
      "• `stats --all` — whole workspace",
      "• `stats --week` / `stats --month` — wider window",
      "• `stats --user @user` — that user's volume (in this channel unless paired with `--all`)",
      "• `stats --channel #other` — a specific channel",
